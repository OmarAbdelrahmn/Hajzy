       ...
       ) : IDABookingService
    {
        ...
        public async Task<Result<BookingResponse>> CreateBookingAsync(CreateBookingRequest request)
        {
            ...
            try
            {
                ...
                var unit = await _context.Units
                    .Include(u => u.Rooms)
                    .FirstOrDefaultAsync(u => u.Id == request.UnitId && u.CityId ==_CurrentDepartmentAdmin.CityId && !u.IsDeleted);
        ...
        public async Task<Result> ConfirmBookingAsync(int bookingId, string adminUserId)
        {
            var booking = await _context.Bookings
                .FirstOrDefaultAsync(b => b.Id == bookingId && b.Unit.CityId == _CurrentDepartmentAdmin.CityId);
            ...
            BookingStatisticsFilter filter)
        {
            var query = _context.Bookings
                .Include(b => b.Unit)
                .Include(b => b.User)
                .Where(b=>b.Unit.CityId == _CurrentDepartmentAdmin.CityId)
                .AsQueryable();
        ...
        public async Task<Result<BookingDetailsResponse>> GetBookingByNumberAsync(string bookingNumber)
        {
            var booking = await _context.Bookings
                ...
                .Include(b => b.Payments)
                .AsNoTracking()
                .FirstOrDefaultAsync(b => b.BookingNumber == bookingNumber && b.Unit.CityId == _CurrentDepartmentAdmin.CityId);
            ...
            BookingFilter filter)
        {
            var query = _context.Bookings
                .Include(b => b.Unit)
                .Where(b => b.UserId == userId && b.Unit.CityId == _CurrentDepartmentAdmin.CityId)
                .AsQueryable();
            ...
            BookingFilter filter)
        {
            var query = _context.Bookings
                .Include(b => b.Unit)
                .Include(b => b.User)
                .Where(b => b.UnitId == unitId && b.Unit.CityId ==_CurrentDepartmentAdmin.CityId)
                .AsQueryable();
            ...
            DateTime checkOut)
        {
            var allRooms = await _context.SubUnits
                .Where(r => r.UnitId == unitId && r.Unit.CityId ==_CurrentDepartmentAdmin.CityId && !r.IsDeleted && r.IsAvailable)
                .ToListAsync();

            var bookedRoomIds = await _context.BookingRooms
                .Include(br => br.Booking)
                .Where(br => br.Room.UnitId == unitId &&
                            br.Room.Unit.CityId == _CurrentDepartmentAdmin.CityId &&
        ...
        public async Task SendBookingConfirmationEmailAsync(int bookingId)
        {
            try
            {
                ...
                var roomDetails = booking.BookingRooms.Where(b=>b.Booking.Unit.CityId == _CurrentDepartmentAdmin.CityId)
