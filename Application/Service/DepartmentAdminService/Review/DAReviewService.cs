        ...
        public async Task<Result<ReviewResponse>> CreateReviewAsync(CreateReviewRequest request)
        {
            try
            {
                ...
                var booking = await _context.Bookings
                    .Include(b => b.Unit)
                    .FirstOrDefaultAsync(b => b.Id == eligibility.BookingId && b.Unit.CityId== _CurrentDepartmentAdmin.CityId);
            ...
            string userId)
        {
            try
            {
                var review = await _context.Set<Domain.Entities.Review>()
                    .Include(r => r.Unit)
                    .Include(r => r.User)
                    .FirstOrDefaultAsync(r => r.Id == reviewId && r.Unit.CityId == _CurrentDepartmentAdmin.CityId);
            ...
            ReviewFilter filter)
        {
            try
            {
                var query = _context.Set<Domain.Entities.Review>()
                    .Include(r => r.User)
                    .Include(r => r.Images)
                    .Where(r => r.UnitId == unitId && r.Unit.CityId == _CurrentDepartmentAdmin.CityId);
            ...
            int pageSize = 20)
        {
            try
            {
                var query = _context.Set<Domain.Entities.Review>()
                    ...
                    .Include(r => r.User)
                    .Include(r => r.Images)
                    .Where(r => r.UserId == userId && r.Unit.CityId == _CurrentDepartmentAdmin.CityId)
            ...
            int unitId)
        {
            ...
            var completedBooking = await _context.Bookings
                .Where(b => b.UserId == userId &&
                           b.UnitId == unitId &&
                           b.Unit.CityId == _CurrentDepartmentAdmin.CityId &&
            ...
            var existingReview = await _context.Set<Domain.Entities.Review>()
                .AnyAsync(r => r.BookingId == completedBooking.Id && r.Unit.CityId == _CurrentDepartmentAdmin.CityId);
            ...
            int pageSize = 20)
        {
            try
            {
                var query = _context.Set<Domain.Entities.Review>()
                    .Include(r => r.Unit)
                    .Include(r => r.User)
                    .Where(r=>r.Unit.CityId == _CurrentDepartmentAdmin.CityId)
                    .OrderByDescending(r => r.CreatedAt);
