    ...
    ILogger<UnitService> logger) : IUnitService
{
        ...
        string userId, UnitFilter filter)
    {
        ...
        var newFilter = new UnitFilter
        {
            Name = filter.Name,
            CityId = userDepartment, // override here
        ...
        return await GetAllComprehensiveAsync(newFilter);
    }
    ...
    public async Task<Result<UnitResponse>> UpdateAsync(int unitId, UpdateUnitRequest request)
    {
        ...
        try
        {
            ...
            if (unit == null)
                return Result.Failure<UnitResponse>(new Error("NotFound", "Unit not found", 404));
    ...
    public async Task<Result> DeleteAsync(int unitId, bool softDelete = true)
    {
        ...
        try
        {
            ...
            if (hasActiveBookings)
                return Result.Failure(
                    new Error("HasActiveBookings", "Cannot delete unit with active bookings", 400));
            ...
            if (softDelete)
            {
                ...
                unit.IsActive = false;
                foreach (var admin in unit.Admins) admin.IsActive = false;
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure(new Error("DeleteFailed", "Failed to delete unit", 500));
        }
    }
    ...
    public async Task<Result> RestoreAsync(int unitId)
    {
        ...
        if (unit == null)
            return Result.Failure(new Error("NotFound", "Deleted unit not found", 404));
    ...
    public async Task<Result> VerifyUnitAsync(int unitId, string adminUserId)
    {
        ...
        unit.UpdatedAt = DateTime.UtcNow.AddHours(3);
        await _context.SaveChangesAsync();
        _logger.LogInformation("Unit {UnitId} verified by admin {AdminId}", unitId, adminUserId);
        return Result.Success();
    }
    ...
    public async Task<Result> UnverifyUnitAsync(int unitId, string adminUserId, string reason)
    {
        ...
        unit.UpdatedAt = DateTime.UtcNow.AddHours(3);
        await _context.SaveChangesAsync();
        _logger.LogInformation("Unit {UnitId} unverified by admin {AdminId}. Reason: {Reason}",
    ...
    public async Task<Result<List<UnitImageResponse>>> UploadImagesAsync(
        int unitId, List<IFormFile> images, string userId)
    {
        using var transaction = await _context.Database.BeginTransactionAsync();
        try
        {
            ...
            var maxOrder = unit.Images.Any() ? unit.Images.Max(i => i.DisplayOrder) : 0;
            ...
            foreach (var (s3Key, index) in uploadResult.Value.Select((k, i) => (k, i)))
            {
                unitImages.Add(new Domain.Entities.UnitImage
                {
                    UnitId = unitId,
            ...
            return Result.Success(unitImages.Select(MapToImageResponse).ToList());
        }
        catch (Exception ex)
        {
            await transaction.RollbackAsync();
    ...
    public async Task<Result> DeleteImageAsync(int unitId, int imageId)
    {
        ...
        try
        {
            var image = await _context.Set<Domain.Entities.UnitImage>()
                .FirstOrDefaultAsync(i => i.Id == imageId && i.UnitId == unitId && !i.IsDeleted);
            ...
            if (image.IsPrimary)
            {
                var nextPrimary = await _context.Set<Domain.Entities.UnitImage>()
                    .Where(i => i.UnitId == unitId && i.Id != imageId && !i.IsDeleted)
    ...
    public async Task<Result> ReorderImagesAsync(int unitId, List<int> imageIds)
    {
        ...
        try
        {
            ...
            foreach (var (imageId, order) in imageIds.Select((id, idx) => (id, idx)))
            {
                ...
                if (image != null) image.DisplayOrder = order;
            }
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure(new Error("UpdateFailed", "Failed to reorder images", 500));
        }
    }
    ...
    public async Task<Result> AttachAdminAsync(int unitId, string userId)
    {
        ...
        try
        {
            ...
            {
                await _context.Set<UniteAdmin>().AddAsync(new UniteAdmin
                {
                    UnitId = unitId,
                    UserId = userId,
                    IsActive = true,
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure(new Error("AttachFailed", "Failed to attach admin", 500));
        }
    }
    ...
    public async Task<Result> RemoveAdminAsync(int unitId, string userId)
    {
        ...
        if (admin == null)
            return Result.Failure(new Error("NotFound", "Admin not found", 404));
    ...
    public async Task<Result<UnitAdminsResponse>> GetUnitAdminsAsync(int unitId)
    {
        ...
        var admins = unit.Admins.Select(a => new UnitAdminInfo(
            ...
            a.User?.PhoneNumber, a.IsActive, a.AssignedAt)).ToList();

        return Result.Success(new UnitAdminsResponse(unit.Id, unit.Name, admins.Count, admins));
    }
    ...
    public async Task<Result<IEnumerable<UnitResponse>>> GetAdminUnitsAsync(string userId)
    {
        ...
        return Result.Success<IEnumerable<UnitResponse>>(units.Select(MapToResponse).ToList());
    }
    ...
    public async Task<Result<IEnumerable<GeneralPolicyResponse>>> GetUnitPoliciesAsync(int unitId)
    {
        ...
        return Result.Success<IEnumerable<GeneralPolicyResponse>>(
            policies.Select(MapToPolicyResponse).ToList());
    }
    ...
    public async Task<Result<UnitStatisticsResponse>> GetStatisticsAsync(int unitId)
    {
        ...
        var occupancyRate = totalBookings > 0
            ? (decimal)completedBookings / totalBookings * 100 : 0;

        return Result.Success(new UnitStatisticsResponse(
            unitId, unit.Name, totalBookings, completedBookings, totalRevenue,
            unit.AverageRating, unit.TotalReviews,
            unit.Rooms.Count(r => !r.IsDeleted), occupancyRate));
    }
    ...
    {
        var units = await _context.Units
            .Include(u => u.City).Include(u => u.UnitType)
            .Include(u => u.Images.Where(i => !i.IsDeleted && i.IsPrimary))
        ...
        decimal latitude, decimal longitude, int radiusKm)
    {
        ...
        var nearby = units.Where(u =>
            CalculateDistance(latitude, longitude, u.Latitude, u.Longitude) <= radiusKm)
            .Select(MapToResponse).ToList();

        return Result.Success<IEnumerable<UnitResponse>>(nearby);
    }
        ...
        int unitId, DateTime checkIn, DateTime checkOut)
    {
        ...
        if (!hasAvailableRooms) return Result.Success(false);
    ...
    public async Task<Result> ToggleFeaturedAsync(int unitId)
    {
        ...
        unit.UpdatedAt = DateTime.UtcNow.AddHours(3);
        await _context.SaveChangesAsync();
        _logger.LogInformation("Unit {UnitId} featured status toggled to {IsFeatured}",
    ...
    public async Task<Result> SetFeaturedAsync(int unitId, bool isFeatured)
    {
        ...
        unit.UpdatedAt = DateTime.UtcNow.AddHours(3);
        await _context.SaveChangesAsync();
        _logger.LogInformation("Unit {UnitId} featured status set to {IsFeatured}",
            unitId, isFeatured);
        return Result.Success();
    }
        ...
        Domain.Entities.Unit unit, List<GeneralPolicy> generalPolicies)
    {
        ...
        var images = unit.Images?.Where(i => !i.IsDeleted)
            ...
            .OrderBy(i => i.DisplayOrder).ToList() ?? new List<UnitImageDetail>();
        ...
        var subUnits = unit.Rooms?.Where(r => !r.IsDeleted).Select(r => new SubUnitComprehensiveDetail
        {
            ...
            Images = r.SubUnitImages?.Where(i => !i.IsDeleted)
                ...
                .OrderBy(i => i.DisplayOrder).ToList() ?? new List<SubUnitImageDetail>(),
    ...
    private static UnitDetailsResponse MapToDetailsResponse(Domain.Entities.Unit unit)
    {
        ...
        var images = unit.Images?.Where(i => !i.IsDeleted)
            ...
            .OrderBy(i => i.DisplayOrder).ToList() ?? new List<UnitImageResponse>();
        ...
        var rooms = unit.Rooms?.Where(r => !r.IsDeleted).Select(r => new SubUnitSummary(
            r.Id, r.RoomNumber, r.SubUnitTypeId, r.PricePerNight, r.MaxOccupancy, r.IsAvailable
    ...
    private static GeneralPolicyResponse MapToPolicyResponse(GeneralPolicy policy)
    {
        return new GeneralPolicyResponse(
            policy.Id, policy.Title, policy.Description, policy.PolicyType,
            policy.PolicyCategory, policy.CustomPolicyName, policy.CancellationPolicyId,
            policy.CancellationPolicy?.Name, policy.IsMandatory, policy.IsHighlighted,
            policy.IsActive, policy.UnitId, policy.Unit?.Name,
            policy.SubUnitId, policy.SubUnit?.RoomNumber, "Unit");
    }
